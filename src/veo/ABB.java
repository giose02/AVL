...
public class ABB implements ABBTDA {
    ...
    System.out.println("Árbol AVL (vertical):");
    imprimirArbolVerticalRec(raiz, "", true);
}

// Método auxiliar recursivo
private void imprimirArbolVerticalRec(NodoAVL nodo, String prefijo, boolean esUltimo) {
    if (nodo != null) {
        System.out.println(prefijo + (esUltimo ? "└── " : "├── ") + nodo.valor);
        String nuevoPrefijo = prefijo + (esUltimo ? "    " : "│   ");
        boolean tieneHijoIzq = nodo.izq != null;
        boolean tieneHijoDer = nodo.der != null;

        if (tieneHijoIzq || tieneHijoDer) {
            if (nodo.izq != null) {
                imprimirArbolVerticalRec(nodo.izq, nuevoPrefijo, nodo.der == null);
            }
            if (nodo.der != null) {
                imprimirArbolVerticalRec(nodo.der, nuevoPrefijo, true);
            }
        }
    }
}
